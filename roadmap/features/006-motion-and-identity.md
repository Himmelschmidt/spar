...
Brings `Tween` and the easing curves with it. `Tween` is `Tween<T>` with `f32` operations only (U46); `HomeRow::Skeleton` reserves rows under the three scan-backed bands while the first cross-project scan is in flight; `gate_zone` is now affordable from 35 columns (`GATE_ZONE_W`+12) instead of 80; the context band's `METER_ZONE_W`/`STEPPER_MIN_W` makes the stepper width a pure function of layout; the rail re-sorts by adjacent-swap travel over `REORDER_PERIOD` (U45) and the tab strip glides over `STRIP_PERIOD` with per-tab `Rect` interpolation (U47). All four U11 shift sources are now closed.
