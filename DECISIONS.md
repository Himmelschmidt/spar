...
| O17 | **Liveness = log recency OR process heartbeat.** A running slot is `stalled` only when both its log and its `bus` heartbeat are quiet past `stall_warn_secs`. A busy-but-log-quiet agent (streaming-json mid tool-call) keeps heartbeating via `LivenessBeat`, so it is not falsely stalled | DECIDED |
