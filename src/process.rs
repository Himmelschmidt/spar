...
pub struct StreamStats {
    ...
    /// for the duration of a recovery spawn, so nothing running concurrently mistakes
    /// the finished turn's id for the recovery turn's, while telemetry consumers that
    /// fall back to this field don't lose the id for good if spar is killed before the
    /// recovery spawn returns and restores it.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
pub fn run_mock(req: &SpawnRequest, mock_output: &str) -> Result<SpawnResult> {
    ...
    // stale-index hazard `reset_api_slot_log` guards against.
