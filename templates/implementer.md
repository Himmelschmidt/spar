...
2. Smoke-check only: compile, typecheck, or 1–2 targeted tests for **your** change (including the acceptance tests if small). Do **not** run the full multi-minute/hour suite — a dedicated suite channel runs that after you finish, either spar's own configured commands or a cheap `tester` slot.
3. Run every build, check, lint and test **in the foreground** and wait for it. No `&`, `nohup`, `disown`, background monitors, or "start it and poll later" patterns. A long build is fine: `timeouts.slot_secs` (90 minutes by default, and most projects raise it) is a **soft** budget, so blocking past it costs you nothing but a reminder to land your work, and the only wall that ends the dispatch sits several times higher. Backgrounding a build and burning your remaining turns polling is how a slot exits without ever writing its summary.
...
6. Write a carry-forward brief to `{{artifacts_dir}}/carry-forward-{{slot_id}}.md` — under 50 lines, and spar truncates it if you go long. If this round does not clear the review, that file is the *only* thing the next round starts with beyond the plan and the contract it already has; the next dispatch is a cold process with an empty context. Write only what a fresh agent cannot cheaply re-derive:
...
7. Write done marker `{{markers_dir}}/{{slot_id}}.done` or `.failed`
