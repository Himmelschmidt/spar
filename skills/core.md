...
- Delivery is per-adapter and you never choose it. **claude** takes nudges through its
  ...
  `/stream/id` line) and its slot is still alive; the same poll file as below is always
  written too, whether or not the push lands, so a nudge is never dependent on the push
  alone. **opencode and codex** have no push channel at all, so
