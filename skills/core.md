...
**`--providers` or `--select`** is required for `plan`, `implement`, and `run` (no silent default fleet) unless every dispatched role is pinned via `--role`.
...
**Run composition (009).** The new-run form is the run's single partial specification. Workflow is optional (`plan` / `implement` / `review` / `arena`) — leaving it unset lets the Chat orchestrator propose one. Each workflow shows its dispatched roles: `plan` is `planner` plus enabled `plan_critic`/`test_author`; `implement` is `implementer` plus the exact reviewer panel plus `tester` only when no deterministic suite replaces it; `review` is the exact reviewer panel; `arena` is a positional pool of `max_agents.max(2)` contenders. The six assignable roles remain the only role rows; `ranker`, `reconciler` and `peer` keep taking fleet positions. Models are opaque free text with local suggestions (`@model` on any `provider@model` ref, stored as storage key plus optional model via the `Pin` boundary — a conflicting `@model` plus separate model is rejected and a double-model ref never emits). `--role ROLE=PROVIDER[@MODEL]` is repeatable and outranks the pool; `--backup ROLE=PROVIDER[@MODEL]` declares a per-role, per-ordinal fallback. A backup fires only for an environmental stop — quota pause, rate limit, or provider unavailable — never on a nonzero work exit, timeout, missing artifact, or adverse review, and it persists as `SeatSource::Backup`. It replaces O52's blind implementer rotation only on that cause and only while not yet consumed; arena never accepts or emits `--role` or `--backup` and retains its positional quota semantics. `--backup` is new-run-only; `plan --run` and `implement --run` without `--reload-config` reject it. A backup whose canonical storage key equals its primary is rejected even when models differ. Operator defaults that prefill a newly opened form live at `registry::spar_home()/defaults.json`, never in the project's `spar.toml`; a run still freezes its resolved fleet into `.spar/runs/<id>/config.json` (O27) so deleting or corrupting defaults after launch cannot alter a continuation.
...
The TUI hosts a resident orchestrator in Main's **Chat** tab (`f(rail selection)` like every other tab): at Home it interviews into a brief, proposes a workflow and fleet with a ````spar-proposal` TOML block that completes the run's partial `RunSpec` (it may propose only blank fields — workflow, task, `[roles]`, `[backups]` — and never silently drops a legacy `providers` list, which is retained visibly until an explicit workflow/role mapping is completed), and launches through the same one-shot `validate_for_launch` + `argv` path a manual form uses (confirmation and manual `Enter` share that validator, so an incomplete spec may open Chat but never spawns an orchestration process). On a selected run it consults the gate against the Review tab's evidence. Transcript is `bus` messages with `surface=chat`, `conversation` and `turn` metadata, rendered as records with immutable bus-event identities. The raw bus palette verb is `:msg`, not `:chat`. Home leads with creation: the first element is the create prompt, with running and waiting runs alongside (amending U7); the `⚑N need you` roll-up, the rail flags and the Gate/Broken toast become the only escalation and are load-bearing, sharing one uncapped `home_needs_you` count that never reads zero while something waits.
...
- **Review artifact schema (enforced):** each `artifacts/review-<slot>.md` is `## Verdict` / `## Acceptance` / `## Findings` / `## Tests` plus a `Reviewed-Commit: <sha>` line naming the `HEAD` the reviewer read (`git rev-parse HEAD` in the review worktree at diff-read time; short shas accepted as a prefix of the panel head). The verdict is read as an **anchored header** — the first non-blank line under the first `## Verdict` must be `approve` or `request_changes`; missing or unparseable is treated as `request_changes`. A verdict naming no commit, or a commit other than the head the panel was dispatched against, is not counted: fail closed exactly like a missing review, with a blocker naming both shas. `## Acceptance` carries one `AC-n: pass|fail|unverified — evidence` line per criterion in `test-contract.md`.
